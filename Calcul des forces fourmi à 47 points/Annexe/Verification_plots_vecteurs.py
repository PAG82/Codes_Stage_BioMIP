# -*- coding: utf-8 -*-
"""
Created on Tue May 30 09:33:29 2023
...
for columns in tableau_donnees_filtrees:     # Permet de lire tout le tableau
    ...
    ### On forme le graph des positions de l'espace selon l'axe Y en fonction de X au cours du temps et on les sauvegarde ###
    
            plt.figure(figsize = (20, 14)) 
            plt.plot(xa_value, ya_value, marker='o', markersize=6, markerfacecolor='blue', label = 'Point proximal')
            plt.plot(xb_value, yb_value, marker='o', markersize=6, markerfacecolor='orange', label = 'Point distal')
            plt.plot(Cg_x_position, Cg_y_position, marker='o', markersize=6, markerfacecolor='red', label = 'Centre de gravité segmentaire')
       

            plt.title('Vérification position du Cg segmentaire')   # titre
            plt.xlabel('position en X')   # axe x
            plt.ylabel('position en Y')    #axe y
            plt.legend()   #légende
            plt.grid()
        
            filename = f"({num_affichage}). {tableau_donnees_filtrees.columns[ya]}, {tableau_donnees_filtrees.columns[yb]}"
            plt.savefig(f'C:/Users/pagro/Desktop/PAS SUPPRIMER/Stage L3 BIOMIP/Travaux PAG/2_Calcul_Accelerations_Cg_segments_47_points/4_Graphiques/2_Verification_position_Cg_segmentaire/{filename}.png') # sauvegarde du graphe
            plt.show()
        
   ### On forme le graph des positions de l'espace selon l'axe Z en fonction de X au cours du temps ###
        
            plt.figure(figsize = (20, 14)) 
            plt.plot(xa_value, za_value, marker='o', markersize=6, markerfacecolor='blue', label = 'Point proximal')
            plt.plot(xb_value, zb_value, marker='o', markersize=6, markerfacecolor='orange', label = 'Point distal')
            plt.plot(Cg_x_position, Cg_z_position, marker='o', markersize=6, markerfacecolor='red', label = 'Centre de gravité segmentaire')
  

            plt.title('Vérification position du Cg segmentaire')   # titre
            plt.xlabel('position en X')   # axe x
            plt.ylabel('position en Z')    #axe y
            plt.legend()   #légende
            plt.grid()
    
            filename = f"({num_affichage}). {tableau_donnees_filtrees.columns[za]}, {tableau_donnees_filtrees.columns[zb]}"
            plt.savefig(f'C:/Users/pagro/Desktop/PAS SUPPRIMER/Stage L3 BIOMIP/Travaux PAG/2_Calcul_Accelerations_Cg_segments_47_points/4_Graphiques/2_Verification_position_Cg_segmentaire/{filename}.png') # sauvegarde du graphe
            plt.show()
